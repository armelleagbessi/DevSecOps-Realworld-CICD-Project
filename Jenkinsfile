...
pipeline{
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@15.188.143.236 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.116.15.7:30000/" || true'
    ...
    always {
        ...
        slackSend channel: 'aa-devsecops-cicd-alerts', //update and provide your channel name
